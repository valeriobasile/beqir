...
def search_expansions(synsets_path, patterns_path, output_folder):
    ...
    for synset, word in synset_list:
        ...
        save_expansions(synset, word, pattern_expansions, output_folder)

def save_expansions(synset, word, pattern_expansions, output_folder):
    expansion_list = []
    ...
    with open(join(dirname(__file__), output_folder, "query_expansions.txt"), "a") as fout:
        fout.write("%s\t%s\t%s\n" % (synset, word, ", ".join(expansion_list)))

    with open(join(dirname(__file__), output_folder, "query_expansions_patterns.txt"), "a") as fout:
        fout.write("%s\t%s\t%s\n" % (synset, word, "\t".join(expasion_pattern_list)))

def process_expansions(patterns_path, input_folder, output_folder):
    all_expansions = {}
    order_expansions = []
    patterns = []
    ...
    with open(join(dirname(__file__), input_folder, "query_expansions_patterns.txt")) as fin:
        for line in fin:
...
def calculate_frequent_expansions(expansions_data, top=10):
    expansions_frequency = {x:[] for x in expansions_data['patterns']}
    words_patterns = []

    for words_data in expansions_data['all_expansions'].values():
        for word, patterns in words_data.items():
            words_patterns.append((word, patterns))

    for word, patterns_data in words_patterns:
        for pattern, expansions in patterns_data.items():
            new_expansions = [x.replace(word, "").strip() for x in expansions]
            expansions_frequency[pattern] += new_expansions
...
def calculate_statistics(input_folder):
    synset_dict = {}
    pattern_dict = {}

    with open(join(dirname(__file__), input_folder, "all_expansions.txt")) as fin:
        for line in fin:
            synset, word, pattern_expansion_list = read_line(line)
...
def create_outputs(expansions_data, output_folder):
    ...
    with open(join(dirname(__file__), output_folder, "all_expansions.txt"), "w") as fout:
        fout.write("\n".join(single_file))

    for index, pattern in enumerate(patterns):
        with open(join(dirname(__file__), output_folder, "pattern_%d.txt" % (index+1)), "w") as fout:
            fout.write("\n".join(multiple_file[pattern]))

    with open(join(dirname(__file__), output_folder, "pattern_names.txt"), "w") as fout:
...
if __name__ == '__main__':
    ...
    raw_data_folder = "../data/output/raw_data/"
    processed_data_folder = "../data/output/processed_data/"
    filtered_data_folder = "../data/output/filtered_data/"

    search_expansions(synsets_path, patterns_path, raw_data_folder)
    expansions_data = process_expansions(patterns_path, raw_data_folder, processed_data_folder)
    filter_expansions(expansions_data, filtered_data_folder)
    calculate_statistics(filtered_data_folder)